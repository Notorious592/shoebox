      ...
      {mode === 'v4' ? (
        <div className="animate-fade-in space-y-6">
            <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                <span className="text-gray-700 font-medium">{t('uuid.count')}:</span>
        ...
        <div className="animate-fade-in space-y-6">
            <div className="space-y-4">
                <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">{t('uuid.input_ph')}</label>
                    <input 
                        type="text"
                        value={contentInput}
                ...
                <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">{t('uuid.ns_ph')}</label>
                    <div className="flex gap-2">
                        <input 
                            type="text"
                            value={namespace}
                            onChange={(e) => setNamespace((e.target as HTMLInputElement).value)}
                            placeholder={`Default: ${DEFAULT_NAMESPACE}`}
            ...
            <div className="space-y-2 pt-4 border-t border-gray-100">
                <label className="block text-sm font-medium text-gray-700">{t('uuid.generated')}</label>
                <div className="relative">
                    <div className="w-full p-4 bg-gray-50 border border-gray-200 rounded-xl font-mono text-primary-700 font-semibold break-all min-h-[56px] flex items-center justify-center text-lg">
                        {contentUuid || <span className="text-gray-400 font-normal text-base italic">{t('common.waiting')}</span>}
                    </div>
                    {contentUuid && (
                        <button 
                            onClick={copyV5}
            ...
            <div className="bg-orange-50 p-4 rounded-lg flex items-start gap-3 text-sm text-orange-800">
                ...
                <div>
                    <p className="font-semibold">{t('uuid.about')}</p>
